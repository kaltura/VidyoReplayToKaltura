...
for ($i = 0; $i < count(Vidyo2KalturaConfig::$conf); $i++) {
    ...
    $ks = $client->generateSession(Vidyo2KalturaConfig::$conf[$i]['ADMIN_SECRET'], 'vidyoSyncClient', KalturaSessionType::ADMIN, Vidyo2KalturaConfig::$conf[$i]['PARTNER_ID']);
    $client->setKs($ks);
    ...
    $pager->pageSize = 1;
    $pager->pageIndex = 1;
    //first test if this account already has such metadata profile (testing for system name == vidyoreplaymetadata)
    ...
    if($results->totalCount == 0) {
        ...
        $configFile = str_replace('\'%metadataprofileid' . $i . '%\'', $metadataProfile->id, $configFile);
